...
class Solution2A(object):
    ...
    def load_data(self):
        ...
        # logging.info("road distance dict is: {}".format(self.road_distance_dict))
        # pprint.pprint(self.road_distance_dict)
        # logging.info("route 2 node info: {}".format(self.route_dict["2"].get_node_set()))
        # logging.info("route 2 transfer node info: {}".format(self.route_dict["2"].get_transfer_info()))
        # invalid_user_set = self.get_invalid_user_set()
        # invalid_train_set = self.get_invalid_train_set()
        # logging.info("invalid user is: {}".format(invalid_user_set))
        # logging.info("invalid user length is: {}".format(len(invalid_user_set)))
        # logging.info("invalid train set is: {}".format(invalid_train_set))
        # logging.info("invalid train set length is: {}".format(len(invalid_train_set)))
    ...
    def get_invalid_train_set(self):
        ...
        for route in self.route_dict.values():
            ...
            for train_number in train_dict:
                # max_arrival_time = self.parse_hms_time("0:00:00")
                # min_arrival_time = self.parse_hms_time("23:59:59")
                ...
                #     min_arrival_time = min(arrival_time, min_arrival_time)
                #
                # delta_time = max_arrival_time - min_arrival_time
                # if delta_time.seconds > 2 * 3600:
                #     print("train_number: {} max_arrival_time: {} min_arrival_time: {} delta_time.seconds: {}".format(
                #         train_number, max_arrival_time,
                #         min_arrival_time,
                #         delta_time.seconds))
                #     invalid_train.add(train_number)
    ...
    def get_path_info(self, start_node_id, start_time, end_node_id, end_time):
        ...
        if route.has_node(start_node_id):
            ...
            for train_number, train_arrival_time, _ in node_train_info_dict[end_node_id]:
                ...
                if start_node_id in train_info_dict[train_number] and \
                        start_time <= train_info_dict[train_number][start_node_id][1]:
                    # 找到两个顶点在同一车次的列车车次
        ...
        nearest_train = route.get_node_k_nearest_train(end_node_id, end_time, 200)
        # logging.info("get possible train: {} in end_node_id: {} at {}".format(nearest_train, end_node_id, end_time))
        ...
        for train_number_possible in nearest_train:
            ...
            for transfer_node_id in transfer_dict:
                ...
                if route.node_in_the_train(transfer_node_id, train_number_possible):
                    # logging.info("transfer node {} in train {}".format(transfer_node_id, train_number_possible))

                    arrival_time, dispatch_time = route.get_node_time_in_train(transfer_node_id, train_number_possible)
                    # 换乘点发车时刻不符合条件
                    if dispatch_time <= start_time or arrival_time >= end_time \
                            or arrival_time <= start_time or arrival_time > end_node_arrival_time:
                        # logging.info(
                        #     "transfer node {} time: {} and {} is not fit, and end_node_arrival_time: {}".format(
                        #         transfer_node_id, arrival_time,
                        #         dispatch_time, end_node_arrival_time))
                        continue
                    ...
                    for transfer_before_node_id in transfer_before_set:
                        ...
                        if solution_path is not None:
                            ...
                            # logging.info("got result in recursive function: {}".format(solution_path))
                            return solution_path
                        else:
    ...
    def rebuild_path(self, path_info_list):
        ...
        for path_info in path_info_list:
            ...
            route = self.route_dict[route_id]
            train_arrival_time_at_start_node = route.get_train_node_arrival_time(train_number, start_node_id)
            ...
            answer_path.append((train_arrival_time_at_start_node, route_name, train_number, end_node_name))
    ...
    def get_path_len_and_change_route_traffic(self, path_info_list):
        ...
        last_end_node_name = None
        # print("path_info_list: {}".format(path_info_list))
    ...
    def get_and_save_all_route_traffic(self):
        ...
        for route_id, route in self.route_dict.items():
            self.route_traffic_info[route_id] = route.get_traffic_value()

        with open(self.route_traffic_file_path, "w+", encoding="UTF-8") as answer_file:
            for route_id, route_traffic in self.route_traffic_info.items():
...
if __name__ == '__main__':
    ...
    # ===== 统计数据 ========
    solution.get_and_save_all_user_path_len()
    solution.get_and_save_all_route_traffic()
    # ==================
