...
class Solution2B(object):
    ...
    def __init__(self,
        ...
        super(Solution2B, self).__init__()
        ...
        # # 用户出行线路长度数据的缓冲区大小
        # self.path_data_buff_size = max(50, self.user_sample_freq / 1000)
        self.path_data_buff_size = 100
    ...
    #         sampled_user_set.add(self.user_list[user_index].user_id)
    #         user_index += user_sample_freq
    #
    #     return sampled_user_set
    ...
    def save_planed_user_path(self, user, plan_path, path_len):
        ...
        """
        if user:
            self.user_path_buff.append((user, plan_path, path_len))

        if not user or len(self.user_path_buff) >= self.path_data_buff_size:
            data_str = ""
            for user, plan_path, path_len in self.user_path_buff:
                data_str += "user_id: {} planed_path: {} path_len: {}\n".format(user.user_id, ", ".join(plan_path), path_len)
            self.answer_file.write(data_str)
